        ...
        for (int j = 0; j < width; j++)
        {
            ...
            for (int a = 0; a < 3; a++)
            {
                for (int b = 0 - 1; b < 3; b++)
                {
                    ...
                    {
                        GxRed += copy[c][d].rgbtRed * Gx[a][b];
                        GxGreen += copy[c][d].rgbtGreen * Gx[a][b];
                        GxBlue += copy[c][d].rgbtBlue * Gx[a][b];

                        GyRed += copy[c][d].rgbtRed * Gy[a][b];
                        GyGreen += copy[c][d].rgbtGreen * Gy[a][b];
                        GyBlue += copy[c][d].rgbtBlue * Gy[a][b];
                    }
                }
            }
            ...
            SobelRGB[0] = round(sqrt(pow((double)GxRed, 2) + pow((double)GyRed, 2)));
            SobelRGB[1] = round(sqrt(pow((double)GxGreen, 2) + pow((double)GyGreen, 2)));
            SobelRGB[2] = round(sqrt(pow((double)GxBlue, 2) + pow((double)GyBlue, 2)));
